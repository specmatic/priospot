...
data class StageTiming(val stage: String, val millis: Long)
...
class PriospotEngine {
    ...
    fun run(config: PriospotConfig): PriospotResult {
        ...
        val coverageReports =
            config.coverageReports.filter { report ->
                val exists = Files.exists(report)
                exists
            }
        val complexityReports =
            config.complexityReports.filter { report ->
                val exists = Files.exists(report)
                exists
            }
        ...
        val complexityMerged =
            timed("ingest-complexity", timings) {
        ...
        val coverageDoc =
            timed("ingest-coverage-normalize", timings) {
                val merged =
                        ...
                        coverageReports
                            ...
                            .map { (path, files) ->
                                ...
                                io.github.priospot.model.CoverageFile(
                                    path = path,
                                    lineCoverage =
                                        io.github.priospot.model
                                            .CoverageCounter(lineCovered, lineTotal),
        ...
        val coverageMerged =
            timed("ingest-coverage-merge", timings) {
                coverageImporter.merge(complexityMerged, coverageDoc)
            }
        val filesWithComplexityFromReport =
            coverageMerged.files.count { file ->
                file.metrics.any { it.name == MetricNames.MAX_CCN }
            }
        val defaultingResult =
            timed("apply-defaults", timings) {
                applyMetricDefaults(
                    coverageMerged,
        ...
        if (defaultingResult.stats.defaultComplexityApplied > 0) {
            diagnostics +=
                "Complexity fallback (MAX-CCN=${config.defaultMaxCcn}) applied to ${defaultingResult.stats.defaultComplexityApplied} files"
        }

        val c3Result =
            timed("compute-c3", timings) {
        ...
        val updatedFiles =
            project.files.map { file ->
                ...
                if (MetricNames.LINE_COVERAGE !in byName) {
                    val (coverageNumerator, coverageDenominator) =
                        if (isTestSourceFile(file.path)) {
                            1.0 to 1.0
                        } else {
                            defaultCoverageNumerator to defaultCoverageDenominator
                        }
                    byName[MetricNames.LINE_COVERAGE] =
                        RatioMetric(
    ...
    private data class DefaultingResult(val project: io.github.priospot.model.Project, val stats: DefaultingStats)
