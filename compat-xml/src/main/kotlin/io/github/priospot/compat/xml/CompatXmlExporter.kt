        ...
        val xml =
            buildString {
                ...
                appendLine(
                    ...
                    )}\" version=\"${escape(document.project.version ?: "")}\" basePath=\"${escape(document.project.basePath)}\">"
                )
                document.project.files.forEach { file ->
                    ...
                    file.metrics.forEach { metric ->
                        when (metric) {
                            is IntegerMetric -> {
                                appendLine(
                                    "      <metric kind=\"integer\" name=\"${escape(metric.name)}\" value=\"${metric.value}\"/>"
                                )
                            }

                            is DecimalMetric -> {
                                appendLine(
                                    "      <metric kind=\"decimal\" name=\"${escape(metric.name)}\" value=\"${metric.value}\"/>"
                                )
                            }

                            is RatioMetric -> {
                                appendLine(
                                    "      <metric kind=\"ratio\" name=\"${escape(
                                        metric.name
